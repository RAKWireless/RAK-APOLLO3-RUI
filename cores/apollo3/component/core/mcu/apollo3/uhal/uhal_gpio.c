...
void uhal_gpio_set_dir(uint32_t pin, gpio_dir_t dir)
{
    ...
    if (dir == GPIO_DIR_OUT) {
      ...
      gpio_status[pin].gpio.eDriveStrength = AM_HAL_GPIO_PIN_DRIVESTRENGTH_2MA;
...
//
//*****************************************************************************
static uint32_t rak_hal_gpio_interrupt_service(uint64_t ui64Status)
{
    ...
    while ( ui32Cnt < 33 )
    {
        ...
        while ( ui32Status )
        {
            ...
            //
            if (sg_gpio_isr[ui32Cnt + ui32FFS] != NULL)
            {
                // Method-1
                //sg_gpio_isr[ui32Cnt + ui32FFS]((uint32_t)ui32Cnt + ui32FFS); // Interrupt Handler...
            
                // Method-2
                //udrv_powersave_in_sleep = false;

                //rui_gpio_int_event[ui32Cnt + ui32FFS].request = UDRV_SYS_EVT_OP_GPIO_INTERRUPT;
                //rui_gpio_int_event[ui32Cnt + ui32FFS].p_context = (void *)ui32Cnt + ui32FFS;
                //udrv_system_event_produce(&rui_gpio_int_event[ui32Cnt + ui32FFS]);

                // Method-3
                pinNumber = ui32Cnt + ui32FFS;
...
am_gpio_isr(void)
{
    ...
    err_code = am_hal_gpio_interrupt_status_get(false, &ui64Status);
    ERROR_CHECK(err_code);
    err_code = am_hal_gpio_interrupt_clear(ui64Status);
    ERROR_CHECK(err_code);
    err_code = rak_hal_gpio_interrupt_service(ui64Status);
    //ERROR_CHECK(err_code);
}
