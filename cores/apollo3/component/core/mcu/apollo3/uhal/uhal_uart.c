...
static SemaphoreHandle_t uart0_semaphore = NULL;
static SemaphoreHandle_t uart1_semaphore = NULL;

static void uart0_take_semaphore(void)
{
    if(uart0_semaphore == NULL)
        uart0_semaphore = xSemaphoreCreateMutex();

    if(isInISR())
    {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if(pdPASS != xSemaphoreTakeFromISR(uart0_semaphore, xHigherPriorityTaskWoken))
        {
            return;
        }
    ...
    {
        if(pdPASS != xSemaphoreTake(uart0_semaphore, portMAX_DELAY))
        {
            return;
        }
    }
...
static void uart0_give_semaphore(void)
{
    if(uart0_semaphore == NULL)
        uart0_semaphore = xSemaphoreCreateMutex();
    ...
    {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if(pdPASS != xSemaphoreGiveFromISR(uart0_semaphore, xHigherPriorityTaskWoken))
        {
            return;
        }
    ...
    {
        if(pdPASS != xSemaphoreGive(uart0_semaphore))
        {
            return;
        }
    }
...
static void uart1_take_semaphore(void)
{
    if(uart1_semaphore == NULL)
        uart1_semaphore = xSemaphoreCreateMutex();
     
    if(isInISR())
    {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if(pdPASS != xSemaphoreTakeFromISR(uart1_semaphore, xHigherPriorityTaskWoken))
        {
            return;
        }
    ...
    {
        if(pdPASS != xSemaphoreTake(uart1_semaphore, portMAX_DELAY))
        {
            return;
        }
    }
...
static void uart1_give_semaphore(void)
{
    if(uart1_semaphore == NULL)
        uart1_semaphore = xSemaphoreCreateMutex();
    ...
    {
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        if(pdPASS != xSemaphoreGiveFromISR(uart1_semaphore, xHigherPriorityTaskWoken))
        {
            return;
        }
    ...
    {
        if(pdPASS != xSemaphoreGive(uart1_semaphore))
        {
            return;
        }
    }
...
static void uart_take_sem(SERIAL_PORT port)
{
    if(port == SERIAL_UART0)
    {
        uart0_take_semaphore();
    }
    else if(port == SERIAL_UART1)
    {
        uart1_take_semaphore();
    }
}

static void uart_give_sem(SERIAL_PORT port)
{
    if(port == SERIAL_UART0)
    {
        uart0_give_semaphore();
    }
    else if(port == SERIAL_UART1)
    {
        uart1_give_semaphore();
    }
}
...
static void uart_init(SERIAL_PORT port, uint32_t baudrate, SERIAL_WIRE_MODE_E WireMode)
{
    ...
    if(uart_status[port].resumed == true)
        return;
...
static void uart_deinit(SERIAL_PORT port)
{
    ...
    if(uart_status[port].resumed == false)
        return;
...
static void uart_write(SERIAL_PORT port, const uint8_t  *buf, int32_t len)
{
    ...
    if(uart_status[port].resumed == false)
    {
        am_log_inf("UART Port%d is not resumed", port);
        return;
    }
