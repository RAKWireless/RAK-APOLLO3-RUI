...
def process(app_file,app_output):
    with open(app_file, mode = 'rb') as f_app:
        app_binarray = f_app.read()
        f_app.close()
    app_crc = crc32(app_binarray)
    print(f'{app_crc:x}')
    with open(app_output + '.bin', mode = 'wb') as out:
        out.write(app_binarray+struct.pack('<L', app_crc))
        #out.write(app_binarray+bytes.fromhex(f'{app_crc:x})


def main():
    process(args.app_file,args.app_output)
    exit()
...
if __name__ == '__main__':
    ...
    parser.add_argument('app_file', help = 'file')

    parser.add_argument('-o', dest = 'app_output', default = 'binary_array',
